...
public class detail extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Picasso.get().load(image).placeholder(R.drawable.ic_baseline_photo_library_24).into(ivImage);
                ...
                myProductsDatabase.child(postId).child("product_likes").child(userId).setValue("1").addOnCompleteListener(new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()) {
                            Toast.makeText(detail.this, "You Liked this product...", Toast.LENGTH_SHORT).show();
                        }
                    }
                });
